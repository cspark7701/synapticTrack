...
class BeamAS2D:
    def __init__(self, data, scan_id=None):
        """
        Initializes a BeamAS object for Alison scanner data.

        Args:
            data (pd.DataFrame): DataFrame with Alison scanner data.
                                 Required columns: ['x_position', 'x_angle', 'x_current', 'hv', 'y_current']
            scan_id (str, optional): Identifier for this scan (e.g. filename, run ID).
        """
        if not isinstance(data, pd.DataFrame):
            raise TypeError("data must be a pandas DataFrame")
        if data.empty:
            raise ValueError("data DataFrame cannot be empty")
        required_columns = ['x_position', 'x_angle', 'x_current']
        if not set(required_columns).issubset(data.columns):
            raise ValueError(f"data must contain columns: {required_columns}")
    ...
    @property
    def x_position(self):
        return self._data['x_position']

    @property
    def x_angle(self):
        return self._data['x_angle']
    ...
    def describe(self):
        ...
        return {
            "scan_id": self._scan_id,
            "points": self.num_points(),
            "angle_range_mrad": (self.x_angle.min(), self.x_angle.max()),
            "current_range_A": (self.x_current.min(), self.x_current.max())
        }
